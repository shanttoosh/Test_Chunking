...
function performQuery() {
    ...
    if (actionButtons) {
        ...
        function resetProcessing() {
            ...
            // Show the 4 configuration boxes again
            const configBoxes = document.getElementById('layer2-config-boxes');
            if (configBoxes) {
                configBoxes.style.display = 'grid';
            }
...
function runProcessingPipelineLayer2() {
    ...
    function processNextStep() {
        if (currentStep >= steps.length) {
            ...
            console.log('Processing completed, calling showQuerySectionLayer2 in 500ms');
            setTimeout(() => {
                console.log('Timeout reached, calling showQuerySectionLayer2');
...
function showQuerySectionLayer2() {
    ...
    // Hide the 4 configuration boxes
    const configBoxes = document.getElementById('layer2-config-boxes');
    console.log('Config boxes element:', configBoxes);
    if (configBoxes) {
        console.log('Hiding config boxes - current display:', configBoxes.style.display);
        configBoxes.style.display = 'none';
        console.log('Config boxes display set to none');
    } else {
        console.log('ERROR: Config boxes not found!');
    }
    
    // Show the query section
    const querySection = document.getElementById('query-section-layer2');
    console.log('Query section element:', querySection);
    if (querySection) {
        console.log('Showing query section - current display:', querySection.style.display);
        querySection.style.display = 'block';
        querySection.style.visibility = 'visible';
        querySection.style.opacity = '1';
        console.log('Query section display set to block');
        // Trigger popup animation
        setTimeout(() => {
            console.log('Adding popup-show class');
            querySection.classList.add('popup-show');
        }, 100);
    } else {
        console.log('ERROR: Query section not found!');
    }
    
    console.log('=== showQuerySectionLayer2 completed ===');
}
