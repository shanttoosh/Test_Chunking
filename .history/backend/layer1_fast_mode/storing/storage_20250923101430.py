...
class FastModeStorage:
    ...
    def store_embeddings(self, embedded_chunks: List, collection_name: Optional[str] = None,
        ...
        try:
            ...
            result = {"success": True, "stored_count": len(embedded_chunks)}
    ...
    def _prepare_storage_data(self, embedded_chunks: List) -> tuple:
        ...
        for i, chunk in enumerate(embedded_chunks):
            ...
            # Build metadata
            metadata = self.metadata_builder.build_chunk_metadata(
                chunk_data=chunk.metadata,
                chunk_id=chunk.id,
                source_file=chunk.metadata.source_file if hasattr(chunk.metadata, 'source_file') else 'unknown',
                chunk_number=i
            )
