...
import { uploadAndProcessCSV, preprocessData, chunkData, generateEmbeddings, storeEmbeddings, searchAPI, getAvailableModels } from './api';
...
function App() {
  ...
  const [showQuerySectionLayer3, setShowQuerySectionLayer3] = useState(false);
  ...
  const [processingSteps, setProcessingSteps] = useState([
    ...
    { id: 'analyze', name: 'Preprocessing', status: 'pending', duration: '', stepText: '', liveSeconds: 0 },
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      // Upload file to backend and get session ID
      const result = await uploadAndProcessCSV(file);
      setSessionId(result.session_id);
      
      // Update CSV preview data
      setCsvPreviewData(result.preview);
  ...
  // Layer 3 Deep Config step timer
  const runStepTimer = async (stepId, stepName, duration = 2000) => {
    const totalSeconds = Math.floor(duration / 1000);
    
    // Set step to executing with live seconds
    updateStepStatus(stepId, 'active', `Executing... 0s`, '', 0);
    setProcessingStep(stepName);
    
    // Live countup
    for (let second = 0; second < totalSeconds; second++) {
      await new Promise(resolve => setTimeout(resolve, 1000));
      updateStepStatus(stepId, 'active', `Executing... ${second + 1}s`, '', 0);
    }
    
    // Set step to completed
    updateStepStatus(stepId, 'completed', `Completed in ${totalSeconds}s`, '', 0);
  };
  ...
  const startProcessing = async () => {
    ...
    try {
      const steps = [
        ...
        { id: 'analyze', duration: 2000, name: 'Preprocessing' },
  ...
  const applyChunkingMethod = async (method, params) => {
    // Mock chunking method application
    console.log(`Applying ${method} chunking`, params);
    await runStepTimer('chunking', 'Applying Chunking Method', 3000);
  ...
  const generateEmbeddings = async (model, batchSize) => {
    // Mock embedding generation
    console.log(`Generating embeddings with ${model}`, batchSize);
    await runStepTimer('embedding', 'Generating Embeddings', 4000);
  ...
  const searchChromaDB = async (query, topK) => {
  ...
  const performQueryLayer3 = async () => {
    if (!query.trim()) {
      alert('Please enter a search query');
      return;
    }
    await searchChromaDB(query, topK);
  };
      ...
      <div className="main-content">
        ...
        {currentLayer === 3 && (
          <Layer3
            ...
            showQuerySection={showQuerySectionLayer3}
            query={query}
            setQuery={setQuery}
            performQuery={performQueryLayer3}
            queryResults={retrievalResults}
            onCloseQuery={() => setShowQuerySectionLayer3(false)}
            render={() => (
              <>
            ...
            <div className="deep-config-container">
              ...
              {deepConfigStep === 1 && (
                <Step1DefaultPreprocess onNext={async () => { 
                  await runStepTimer('preprocess', 'Applying Default Preprocessing', 2500);
                ...
                <div className="config-step" data-step="2">
                  ...
                  <div className="step-actions">
                    ...
                    <button 
                      ...
                      onClick={async () => {
                        await runStepTimer('preprocess', 'Applying Type Conversion', 2000);
                        setDeepConfigStep(3);
                        scrollToNextStep(3);
                      }}
                    >
                  ...
                  <div className="step-actions">
                    ...
                    <button 
                      ...
                      onClick={async () => {
                        ...
                        await runStepTimer('preprocess', 'Applying Null Handling', 2000);
                        setDeepConfigStep(4);
                      }}
                    >
                      Apply Null Handling
                ...
                <div className="config-step" data-step="4">
                  ...
                  <div className="step-actions">
                    ...
                    <button 
                      className="btn btn-primary" 
                      onClick={async () => {
                        await runStepTimer('preprocess', 'Applying Duplicate Handling', 2000);
                  ...
                  <div className="step-actions">
                    ...
                    <button 
                      ...
                      onClick={async () => {
                        ...
                        // Run timer for text processing
                        await runStepTimer('preprocess', 'Applying Text Processing', 3000);
              ...
              {deepConfigStep === 7 && (
                <Step7Chunking
                  ...
                  onApply={async () => { await applyChunkingMethod(selectedChunkingMethod, {}); }}
                />
              )}
  ...
  "semantic_coherence": "High"
              ...
              {deepConfigStep === 9 && (
                <Step9Embedding
                  ...
                  onNext={async () => { await generateEmbeddings(embeddingModelChoice, embeddingBatchSize); }}
                />
              )}
              ...
              {deepConfigStep === 11 && (
                <Step11Storage
                  ...
                  onStore={async () => {
                    await runStepTimer('storage', 'Storing Vectors', 3000);
              ...
              {deepConfigStep === 12 && (
                <Step12Retrieval
                  ...
                  onSearch={async () => { 
                    setShowQuerySectionLayer3(true);
                  }}
                  onComplete={() => setShowQuerySectionLayer3(true)}
                />
              )}
            </div>
            </>
