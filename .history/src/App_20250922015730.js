...
function App() {
  ...
  const [processingSteps, setProcessingSteps] = useState([
    { id: 'upload', name: 'File Upload', status: 'pending', duration: '', stepText: '' },
    { id: 'analyze', name: 'Data Analysis', status: 'pending', duration: '', stepText: '' },
    { id: 'preprocess', name: 'Preprocessing', status: 'pending', duration: '', stepText: '' },
    { id: 'chunking', name: 'Chunking', status: 'pending', duration: '', stepText: '' },
    { id: 'embedding', name: 'Embedding', status: 'pending', duration: '', stepText: '' },
    { id: 'storage', name: 'Storing', status: 'pending', duration: '', stepText: '' },
    { id: 'retrieval', name: 'Retrieval', status: 'pending', duration: '', stepText: '' }
  ]);
  ...
  const handleFileUpload = (event) => {
    ...
    updateStepStatus('upload', 'completed', '1s', '✓ Completed');
  };

  // Update step status
  const updateStepStatus = (stepId, status, duration = '', stepText = '') => {
    setProcessingSteps(prev => prev.map(step => 
      step.id === stepId ? { ...step, status, duration, stepText } : step
    ));
  };
  ...
  const startProcessing = async () => {
    ...
    const steps = [
      ...
      { id: 'storage', duration: 1500, name: 'Storing Vectors' },
      { id: 'retrieval', duration: 1000, name: 'Testing Retrieval' }
    ];
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      updateStepStatus(step.id, 'active', '', '⏳ Executing...');
      ...
      updateStepStatus(step.id, 'completed', `${Math.floor(step.duration / 1000)}s`, '✓ Completed');
    }
  ...
  const resetProcessing = () => {
    ...
    setProcessingSteps(prev => prev.map(step => ({ ...step, status: 'pending', duration: '', stepText: '' })));
  };
        ...
        <div className="sidebar show">
            ...
            {processingSteps.map((step) => (
              <div key={step.id} className={`process-step ${step.status}`}>
                <div className="step-content">
                  ...
                  <div className="step-status">{step.stepText}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="progress-section">
            ...
            <div className="progress-text">
              {isProcessing ? (processingStep || 'Processing...') : 'Processing Complete!'}
            </div>
          </div>
