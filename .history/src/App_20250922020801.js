...
function App() {
  ...
  const handleFileUpload = (event) => {
    ...
    updateStepStatus('upload', 'completed', '1s', 'Completed', 0);
  };
  ...
  const startProcessing = async () => {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      updateStepStatus(step.id, 'active', '', 'Executing', 0);
      ...
      for (let second = 0; second < totalSeconds; second++) {
        ...
        updateStepStatus(step.id, 'active', '', 'Executing', second + 1);
      }
      
      // Set step to completed
      updateStepStatus(step.id, 'completed', `${totalSeconds}s`, 'Completed', 0);
    }
  ...
  const startProcessingLayer2 = async () => {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      // Set step to executing with live seconds
      updateStepStatus(step.id, 'active', '', 'Executing', totalSeconds);
      setProcessingStep(step.name);
      setProcessingProgress(progress);
      ...
      for (let second = totalSeconds; second > 0; second--) {
        await new Promise(resolve => setTimeout(resolve, 1000));
        updateStepStatus(step.id, 'active', '', 'Executing', second);
      }
              ...
              <div key={step.id} className={`process-step ${step.status}`}>
                <div className="step-content">
                  ...
                  <div className="step-timing">
                    {step.status === 'active' ? `${step.liveSeconds}s` : step.duration}
                  </div>
