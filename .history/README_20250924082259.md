...
4. **"File upload failed" error**:
   ...
   - Check that frontend API config uses `http://127.0.0.1:8001`
   - Verify backend server is accessible: `curl http://127.0.0.1:8001/health`
   - Check browser console for CORS errors

5. **JSON serialization errors**:
   - Restart the backend server after code changes
   - Check for NaN or infinite values in CSV data
...
1. **Verify Backend is Running**:
   ...
   # Should return: {"message": "CSV Chunking Optimizer API"}
   ```

2. **Check Frontend API Configuration**:
   - Open `src/api/index.js`
   - Verify `BASE_URL: 'http://127.0.0.1:8001'`
   - Check browser Network tab for failed requests

3. **Test API Endpoints**:
   - Visit http://127.0.0.1:8001/docs
   - Try uploading a CSV file through the API docs
