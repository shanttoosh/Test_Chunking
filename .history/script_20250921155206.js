...
function checkAndShowProcessingPipeline() {
    console.log('checkAndShowProcessingPipeline called');
    ...
    if (layerSelected && fileUploaded) {
        ...
        if (currentLayer === 1) {
            ...
            if (pipelineSection) {
                pipelineSection.classList.add('show');
                console.log('Layer 1 pipeline shown');
            }
        } else if (currentLayer === 2) {
            // Layer 2
            const pipelineSectionLayer2 = document.getElementById('processing-pipeline-section-layer2');
            console.log('Layer 2 pipeline section found:', pipelineSectionLayer2 ? 'Yes' : 'No');
            if (pipelineSectionLayer2) {
                pipelineSectionLayer2.classList.add('show');
                console.log('Layer 2 pipeline shown');
            }
        }
    } else {
        console.log('Conditions not met for showing pipeline');
    }
}
...
function selectLayer(layer) {
    ...
    console.log('Found layer cards:', layerCards.length);
    layerCards.forEach((card, index) => {
        const shouldBeActive = index + 1 === layer;
        card.classList.toggle('active', shouldBeActive);
        console.log(`Layer card ${index + 1}: active = ${shouldBeActive}`);
    });
    ...
    contentSections.forEach((section, index) => {
        ...
        console.log(`Content section ${index + 1}: active = ${shouldBeActive}, id = ${section.id}`);
    });
...
function performQueryLayer2() {
    ...
    // Generate mock results for Layer 2 (matching Layer 1 format)
    const mockResults = [
        {
            title: "Chunk 1",
            content: `Customer data analysis showing purchasing patterns across different demographics and regions. This chunk contains detailed information about customer behavior, preferences, and buying trends that can help optimize marketing strategies and product development.`
